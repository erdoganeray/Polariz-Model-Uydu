...
struct __attribute__((packed)) TelemetryPacket {
  ...
  // RTC verileri
  String gonderme_saati;  //Daha sonra değişecek. //gonderme_saati
...
void collectAndSendData() {
  ...
  if (rtc_status) {
    ...
    if (now.year() > 2020 && now.year() < 2035) {  // Makul tarih aralığı kontrolü
      data.gonderme_saati = String(now.year()) + "-" + 
                       String(now.month()) + "-" + 
                       String(now.day()) + " " + 
                       String(now.hour()) + ":" + 
                       String(now.minute()) + ":" + 
      ...
      rtc_status = false;
      data.gonderme_saati = "0000-00-00 00:00:00";
...
void loadLastPacketNumber() {
  ...
  if (lastLine.length() > 0 && lastLine != "Packet_Number,Uydu_Statusu,Hata_Kodu,gonderme_saati,Pressure1,Pressure2,Yukseklik1,Yukseklik2,Irtifa_Farki,Inis_Hizi,Temperature,Pil_Gerilimi,gps1_latitude,gps1_longitude,gps1_altitude,Pitch,Roll,Yaw,RHRH,IoT_S1_Data,IoT_S2_Data,Takim_No,Sensor_Status") {
